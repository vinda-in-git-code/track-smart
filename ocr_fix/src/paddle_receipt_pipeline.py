        ...
        try:
            ...
            if section == "HEADER":
                if price is not None:
                    section = "ITEMS"
                    if args.debug: print(f"[DEBUG] Reason: Found price {price}, switching to ITEMS")
                else:
                    pending_name = text
                    if args.debug: print("[DEBUG] Reason: No price found, keeping as pending name")
                    continue

            if section == "ITEMS":
                if is_stop_marker(text):
                    section = "FOOTER"
                    if args.debug: print("[DEBUG] Reason: Found stop marker, switching to FOOTER")
                    continue
                    
                if is_strict_non_item(text):
                    if args.debug: print("[DEBUG] Reason: Strict NON_ITEM matched, discarding line")
                    continue
            ...
            if section == "FOOTER":
                if args.debug: print("[DEBUG] Accepted: NO | Reason: In FOOTER section")
                
        except Exception as e:
            if args.debug: print(f"[DEBUG] Error processing line: {e}")
            continue
